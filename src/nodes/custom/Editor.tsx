...
import { slugifyKey, type PrefabField, type PrefabFieldKind } from "../../../shared/prefab";
...
function uniqueKey(base: string, fields: PrefabField[]): string {
...
export function CustomEditor({ data, onChange }: NodeEditorProps<CustomNodeData>) {
  ...
  const setFields = (fields: PrefabField[]) => onChange({ ...data, fields });
  ...
  const setKind = (index: number, kind: PrefabFieldKind) => {
      ...
      {data.fields.map((f, index) => (
        <div key={f.key} className="custom-field">
          <div className="editor-row">
            ...
            <label>
              Kind
              <select value={f.kind} onChange={(e) => setKind(index, e.target.value as PrefabFieldKind)}>
                {KINDS.map((k) => (
