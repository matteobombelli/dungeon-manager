...
function tokenOf(cookie: string): string {
  return cookie.slice("dm_session=".length);
}
...
describe("POST /auth/register", () => {
  it("creates the user and sets the session cookie", async () => {
    ...
    expect(cookie).toMatch(/^dm_session=[A-Za-z0-9_-]{43}; /);
    expect(cookie).toContain("HttpOnly");
    expect(cookie).toContain("SameSite=Lax");
    expect(cookie).toContain("Path=/projects/dungeon-manager");
...
describe("POST /auth/login", () => {
  ...
  it("returns the user and a new session cookie", async () => {
    ...
    expect(setCookies(res)[0]).toContain("Max-Age=2592000");
...
describe("GET /auth/me", () => {
  ...
  it("rejects an unknown token", async () => {
    ...
    const res = await request("/auth/me", { headers: cookieHeader("dm_session=garbage") });
    expect(res.status).toBe(401);
  });
  ...
  it("rejects an expired session and removes it", async () => {
    ...
    await env.DB.prepare("INSERT INTO sessions (id, user_id, created_at, expires_at) VALUES (?, ?, ?, ?)")
      .bind(id, user.id, Date.now() - 31 * DAY, Date.now() - DAY)
      .run();

    const res = await request("/auth/me", { headers: cookieHeader(`dm_session=${token}`) });
    expect(res.status).toBe(401);
    const row = await env.DB.prepare("SELECT id FROM sessions WHERE id = ?").bind(id).first();
    expect(row).toBeNull();
  });

  it("rolls a session with less than 15 days left", async () => {
    ...
    expect(row?.expires_at).toBeGreaterThanOrEqual(before + 30 * DAY);
    expect(row?.expires_at).toBeLessThanOrEqual(Date.now() + 30 * DAY);
  });
});
...
describe("POST /auth/logout", () => {
  it("deletes the session and clears the cookie", async () => {
    ...
    const cookies = setCookies(res);
    expect(cookies).toHaveLength(1);
    expect(cookies[0]).toMatch(/^dm_session=; /);
    expect(cookies[0]).toContain("Max-Age=0");
    expect(cookies[0]).toContain("Path=/projects/dungeon-manager");
