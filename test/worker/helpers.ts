...
export const API = "https://apps.matteob.dev/projects/dungeon-manager/api";
...
/** The "dm_session=<value>" pair from a Set-Cookie header, ready to send back as Cookie. */
export function sessionCookie(res: Response): string {
  const setCookie = res.headers.get("Set-Cookie");
  if (!setCookie) throw new Error("no Set-Cookie header on response");
  const pair = setCookie.split(";")[0];
  if (!pair.startsWith("dm_session=")) throw new Error(`unexpected Set-Cookie: ${setCookie}`);
  return pair;
}
